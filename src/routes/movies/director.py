...
async def create_director(director: BaseCreateSchema, director_service: DirectorService = Depends(get_director_service)):
...
async def read_director(director_id: int, director_service: DirectorService = Depends(get_director_service)):
...
async def read_directors(director_service: DirectorService = Depends(get_director_service)):
    return await director_service.get_all_directors()

@router.delete("/{director_id}", response_model=BaseResponseSchema)
async def delete_director(director_id: int, director_service: DirectorService = Depends(get_director_service)):
